...
class Calc555Resistors < Minitest::Test
  def test_simple_two_part
    assert_equal ['Mary had a', 'little lamb'],
                 TextWrapper.wrap('Mary had a little lamb', 2)
  end

  def test_default_two_part
    assert_equal ['Mary had a', 'little lamb'],
                 TextWrapper.wrap('Mary had a little lamb')
  end

  def test_simple_three_part
    assert_equal ['Mary had a little', 'lamb, its fleece', 'was White as snow'],
                 TextWrapper.wrap(
                   'Mary had a little lamb, its fleece was White as snow', 3)
  end
  ...
  def test_large_text_into_eight_pieces
    assert_equal ['When run directly, it starts by initializing the panel', # 54
                  ...
                  'to hit enter again and then disconnecting from GPIO.'],
                 TextWrapper.wrap( # 51
  ...
  def test_right_bias
    assert_equal ['xxxxxx xxxxxx', 'xxxxxx'],
                 TextWrapper.wrap('xxxxxx xxxxxx xxxxxx', 2)
  end
end
